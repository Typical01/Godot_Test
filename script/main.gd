...
@onready var hud = $Hud
@onready var color_rect_map = $ColorRect
@onready var joystick = $Hud/GamePlayingUi/Sprite2DJoystick
var score
var speed
var follow_touch
...
func _on_start_timer_timeout() -> void:
	...
	if start_wait_time != 0:
		audio_stream = load("res://dodge_the_creeps_2d_assets/art/CF/幽灵出现倒计时_%s.wav" % (start_wait_time))
		hud.show_message(str(start_wait_time))
	if start_wait_time > 0 and !$SoundGhostRefreshCountDown.is_playing():
		if audio_stream: $SoundGhostRefreshCountDown.stream = audio_stream
...
func _initialize_player() -> void:
	if player and hud:
		player.activity_range = color_rect_map.size
		print("玩家活动范围: ", player.activity_range)
...
func _on_hud_sound_change(volume_percentage: float) -> void:
	...
	set_sound_volume(sound_volume, true)
	pass # Replace with function body.

func set_sound_volume(volume_percentage: float, is_save_data = false) -> void:
	...
	if is_save_data and  !save_data(): print("Game: sound_volume: 保存数据失败!")
	pass

func _on_hud_music_change(volume_percentage: float) -> void:
	music_volume = volume_percentage
	set_music_volume(music_volume, true)
	pass # Replace with function body.

func set_music_volume(volume_percentage: float, is_save_data = false) -> void:
	$Music.set_volume_linear(volume_percentage)
	print("设置音乐音量: ", volume_percentage)
	if is_save_data and  !save_data(): print("Game: music_volume: 保存数据失败!")
	pass
...
func _on_hud_follow_touch_change(change: bool, is_save_data = false) -> void:
	follow_touch = change
	if is_save_data and  !save_data(): print("Game: follow_touch: 保存数据失败!")
	pass # Replace with function body.
