...
var size: Vector2:
	get():
		return Vector2(data.dimentions.x, data.dimentions.y) * Global.SLOT_SIZE
...
func _ready() -> void:
	...
	var quality_color = Global.Quality.Red
	self_modulate = get_color(quality_color)
	quality_border.modulate = get_color(quality_color)
	audio_player.stream = load("res://art/item_slot/音效/%s.wav" % get_color_from_string(Global.Quality.Red))
...
func set_all_scale(scale_num):
	...
	item_name.position -= rect.size / 2 - Vector2(2, 0.5)
...
func get_picked_up() -> bool:
	...
	self_modulate = get_color(Global.Quality.Red, 0.0)
...
func get_placed(pos: Vector2i) -> void:
	audio_player.play()
	self_modulate = get_color(Global.Quality.Red, 0.28)
...
func get_color(quality_color: Global.Quality, alpha: float = 0.28) -> Color:
	match quality_color:
		Global.Quality.White:
			return Color(0.2, 0.2, 0.2, alpha)
		Global.Quality.Green:
			return Color(0.02, 0.1, 0.02, alpha)
		Global.Quality.Blue:
			return Color(0.05, 0.2, 0.4, alpha)
		Global.Quality.Purple:
...
func get_color_from_string(quality_color: Global.Quality) -> String:
	match quality_color:
		Global.Quality.White:
			return "白"
		Global.Quality.Green:
